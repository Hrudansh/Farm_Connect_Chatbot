...
genai.configure(api_key="AIzaSyBW6X18vlEZdanxHAHAQAettdlk3bcKjIY")  # Replace <API-KEY> with your actual API key
model = genai.GenerativeModel('gemini-1.5-flash')

def generate_plant_disease_info(disease_name, language):
    # Constructing the prompt based on the disease name and selected language
    prompt = f"""Provide detailed information about the plant disease '{disease_name}' in {language}. The response should cover the following:
...
def plant_disease_chatbot():
    ...
    language = st.selectbox("Select Language", ["English", "Marathi", "Hindi"], key="language_select")
    ...
    if user_input:
        ...
        try:
            ...
            response = generate_plant_disease_info(user_input, language)
